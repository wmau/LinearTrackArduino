...
//Data in the resulting single .txt file will be listed in the format {XXXX, session, YYYY, lickC, ZZZ, lickL, etc.).
...
//this should detect if the sensor on 11 was touched and if so initiate recording that event. We will use this sensor to tell us when we started and stopped the session (2 events) by manually pressing it.
